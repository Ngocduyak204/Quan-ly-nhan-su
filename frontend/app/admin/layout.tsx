...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex">
      {/* Sidebar */}
      <aside className="w-64 bg-slate-900/60 border-r border-slate-800 p-4 flex flex-col justify-between hidden md:flex shrink-0">
        <div>
          <div className="flex items-center gap-3 px-2 mb-8">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-blue-600 to-purple-600 flex items-center justify-center font-bold text-lg shadow-lg shadow-purple-500/20">
              Q
            </div>
            <div>
              <h1 className="font-extrabold text-base tracking-wide text-white">QLNV-SL Admin</h1>
              <p className="text-xs text-purple-400 font-medium">Giao diện quản trị</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="p-4 bg-slate-900 border-b border-slate-800 flex items-center justify-between md:hidden">
          <span className="font-bold text-sm text-purple-300">QLNV-SL Admin</span>
          <button onClick={logout} className="text-xs text-red-400 font-semibold">Đăng xuất</button>
        </header>
